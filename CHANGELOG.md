...
- **`agentverse-memory` skill — JWT/shared-space auth flow + response-shape reference for all 35 tools** (skill `version` → 1.3.0). Two additions that complete the offline reference: (1) a **Response Shapes (all 35 tools)** section documenting the top-level `structuredContent` keys each tool returns (verified against the live server); (2) a **Shared Spaces & JWT Authentication** section. The 5 shared-space tools require a JWT (`Authorization: Bearer <jwt>`) — and there is **no public self-serve JWT endpoint** (`POST /v1/keys` issues only API keys; JWTs are HS256 signed with the server's `JWT_SECRET`, held by the Agentverse/ASI identity layer), so shared spaces are **not self-serve with a free API key**. Documents the exact JWT claim shape (`sub` / `iss=agentverse-memory` / `aud=am-server` / `spaces` / `tier` / `exp` / `iat`), the in-band `-32001 unauthorized` error returned to API-key-only callers, and the owner→issue-JWT→join→store/query flow for integrators who hold the secret. Also corrected the stale `memory_join_shared_space` "invite token" description (it joins by `space_id`, JWT-granted).
